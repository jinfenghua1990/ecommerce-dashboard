...
def create(body: CloseBody, db: Session = Depends(get_db)) -> dict[str, Any]:
    try:
        row = svc.create_snapshot(db, body.year, body.month)
...
@router.post("/recalc")
def recalc(body: CloseBody, db: Session = Depends(get_db)) -> dict[str, Any]:
    """重新计算某月：V1 保留，产生 V{n+1}（规格 1.6）。"""
    try:
        row = svc.recalc(db, body.year, body.month)
    except ValueError as exc:
        raise HTTPException(400, str(exc))
    return {"id": row.id, "version": row.version, "period": f"{body.year}-{body.month:02d}",
