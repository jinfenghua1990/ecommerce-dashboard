...
from app.services.procurement_workbench_service import _consumable_source_nos, _order_kind
...
def _effective_purchase_status(row: dict[str, Any]) -> str:
    ...
    if status in _STAGE_META:
        return status
    # 历史/文件订单可能还没写采购状态，但已经存在真实入库关联；事实优先。
    if row.get("inbound"):
        return "inbound"
...
def list_production_purchase_rows(
    ...
    *,
    ...
    return {"rows": rows, "summary": summary, "group": group}
