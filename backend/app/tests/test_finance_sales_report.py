...
def test_monthly_sales_report_uses_valid_sales_and_does_not_duplicate_order_values(db_session):
    ...
    db_session.add_all([
        ...
        AftersalesOrder(
            aftersale_no=_order_no("AFTER"),
            order_no=order_no,
            type="refund",
            status="已完成",
            refund_amount=Decimal("20"),
            created_at_src=datetime(2026, 8, 20, 10, 0, tzinfo=tz),
        ),
        AftersalesOrder(
            aftersale_no=_order_no("CANCEL"),
            order_no=order_no,
            type="refund",
            status="已取消",
            refund_amount=Decimal("500"),
            created_at_src=datetime(2026, 8, 21, 10, 0, tzinfo=tz),
        ),
    ])
    db_session.flush()
...
def test_refund_can_be_disabled_by_template_rule(db_session):
    ...
    template = svc.save_template(
        ...
        company=company,
        fields=svc.default_fields(),
        rules={"refund_mode": "ignore_refund"},
