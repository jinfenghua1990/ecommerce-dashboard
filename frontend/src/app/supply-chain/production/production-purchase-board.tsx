    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-100 px-5 py-4">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-3 xl:grid-cols-6">
          ...
          <Metric label="待生产/生产中" value={summary.productionCount} />
            ...
            {rows.map((row) => (
              <tr key={`${row.platform}:${row.orderNo}`} className="align-top hover:bg-slate-50/60">
                ...
                <td className="px-4 py-3 text-right">
                  {row.orderId !== null ? (
                    <Link href={`/purchase/workbench?order=${row.orderId}`} className="inline-flex rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-700">
                      查看采购主单
                    </Link>
