...
export default function ReceivingPage() {
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-4">
      <header className="sticky top-0 z-20 -mx-8 -mt-6 border-b border-slate-200 bg-white/95 px-8 py-4 backdrop-blur">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <div className="flex gap-2">
            <Link href="/supply-chain/in-transit" className="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-600 hover:bg-slate-50">生产执行与在途</Link>
            <Link href="/products/inventory-goods" className="rounded-lg bg-indigo-600 px-3 py-2 text-sm font-medium text-white hover:bg-indigo-700">正品库存</Link>
          </div>
        </div>
      ...
      <div className="rounded-lg border border-amber-100 bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-800">确认关联只表示“这批生产到货对应这张吉客云真实入库单”，本系统不会自行增加正品库存，避免与吉客云库存快照重复计算。</div>
    </div>
  );
}
