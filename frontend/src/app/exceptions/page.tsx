...
const STATUS_STYLE: Record<string, string> = {
  ...
  ignored: "bg-gray-100 text-gray-500 ring-gray-200",
  ...
  return (
    <div>
          ...
          {["pending", "confirmed", "resolved", "ignored", "all"].map((s) => (
            <button
              ...
              className={`rounded-lg px-2.5 py-1 text-xs ${
                filter === s ? "bg-indigo-600 text-white" : "bg-white text-gray-600 ring-1 ring-gray-200"
              }`}
            >
      ...
      <div className="mt-4 overflow-hidden rounded-xl border border-gray-200 bg-white">
        {rows.length === 0 ? (
          <div className="p-8 text-center text-sm text-gray-400">当前筛选下没有异常记录</div>
        ) : (
              ...
              {rows.map((r) => (
                <tr key={r.id}>
                  <td className="px-4 py-2.5">
                    <span className={r.severity === "high" ? "text-red-600" : "text-gray-500"}>
